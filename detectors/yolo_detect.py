...
class Yolov5Tflite:
    ...
    def detect_test():
        ...
        original_size = image.shape[:2]
        input_data = np.ndarray(shape=(1, 416, 416, 3),
                                dtype=np.float32)
        image = cv2.resize(image, (416, 416))
        input_data[0] = image.astype(np.float32)/255.0
        input_data[0] = image
        interpreter = tf.lite.Interpreter('models/yolov5s-fp16.tflite')
        interpreter.allocate_tensors()
        ...
        result_boxes, result_scores, result_class_names = y5s.nms(prediction=pred)

        # Filter out predictions below a certain confidence threshold
        confidence_threshold = 0.5
        filtered_boxes = [box for i, box in enumerate(result_boxes) if result_scores[i] > confidence_threshold]

        erson_class_index = y5s.names.index("person")
        filtered_boxes = [box for i, box in enumerate(filtered_boxes) if result_class_names[i] == "person"]
